...
/**
 * Employee Portal Servlet
 *
 * @author Ankith Jain
 */
@WebServlet(name = "EmployeePortal", urlPatterns = {"/employeePortal"})
public class EmployeePortal extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet EmployeePortal</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Welcome EmployeePortal at " + request.getContextPath() + "</h1>");
            if (request.isUserInRole("employee")) {
                Employee e = employeeService.findByUsername(request.getRemoteUser());
                out.println("<h1>" + e.getEmp_first_name() + "<br/>" + e.getEmp_last_name() + "</h1>");
            }
